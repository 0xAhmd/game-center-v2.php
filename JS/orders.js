...
function renderOrders(orders, asAdmin) {
  ...
  orders.forEach(o => {
    ...
    html += `<tr>
      ...
      <td><button class="btn-details" onclick="viewOrderDetail(${o.id}, '${o.status}')">Details</button></td>
         <td class="text-white">
        ${o.item_count} game${o.item_count != 1 ? 's' : ''}
        ${o.game_titles ? `<div style="font-size:.75rem;color:rgba(255,255,255,.4);margin-top:3px;max-width:220px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="${escAttr(o.game_titles)}">${escHtml(o.game_titles)}</div>` : ''}
      </td>
