// JS/library.js — User Game Library
...
fetch('../features/auth/session_status.php')
  ...
  .then(s => {
    ...
    if (s.role === 'admin') { window.location.href = 'index.html'; return; }
    loadLibrary();
  })
  .catch(() => { window.location.href = 'login.html'; });

function loadLibrary() {
  fetch('../features/profile/get_user_library.php')
    .then(r => r.json())
    .then(data => {
      _allGames = data;
      updateCountBadge();
      document.getElementById('libSortBar').style.display = data.length ? '' : 'none';
      renderLibrary();
    })
    .catch(() => {
...
function renderLibrary() {
  ...
  if (_sortMode === 'title') games = games.sort((a, b) => a.title.localeCompare(b.title));
  else if (_sortMode === 'price') games = games.sort((a, b) => parseFloat(b.price) - parseFloat(a.price));
  ...
  if (!games.length && !_allGames.length) {
    grid.innerHTML = `
      <div class="col-12 lib-empty">
        ...
        <p class="text-white-50 mt-2">Head to the store and add some games!</p>
...
function buildCard(g) {
  ...
  const date  = new Date(g.purchase_date).toLocaleDateString('en-US', { month:'short', day:'numeric', year:'numeric' });
  const desc  = g.description || 'No description available.';

  return `
<div class="col-md-4 col-sm-6 lib-col" data-game-id="${g.game_id}">
  <div class="lib-card">
    ...
    <div class="lib-card-body">
      <div class="lib-card-title">${escHtml(g.title)}</div>
      ...
      <div class="lib-card-footer">
        ...
        <div class="d-flex align-items-center gap-2">
          <button class="btn-play" id="playBtn_${g.game_id}"
                  onclick="handlePlay(${g.game_id}, '${escAttr(g.title)}')">▶ Play</button>
          <button class="btn-remove-lib" onclick="handleRemove(${g.game_id}, '${escAttr(g.title)}')">✕</button>
        </div>
      </div>
    </div>
  </div>
...
function handlePlay(gameId, title) {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => { btn.classList.remove('launched'); btn.innerHTML = '▶ Play'; }, 4000);
  }, 1500);
}
...
function handleRemove(gameId, title) {
  ...
  fetch('../features/profile/remove_from_library.php', { method: 'POST', body: fd })
    .then(r => r.json())
    .then(d => {
      if (d.success) {
        ...
        document.getElementById('libSortBar').style.display = _allGames.length ? '' : 'none';
      } else {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (input) input.addEventListener('input', renderLibrary);
});
...
function showLibToast(type, title, msg, duration = 3500) {
  ...
  setTimeout(() => { t.classList.remove('visible'); setTimeout(() => t.remove(), 320); }, duration);
}
